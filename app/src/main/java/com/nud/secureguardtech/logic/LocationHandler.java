...
public class LocationHandler {
    ...
    public void newLocation(String provider, String lat, String lon) {
        ...
        msg = String.format("%s:\n\n%s\n\n%s",
                "Phone's Location (Coordinates)", (lat + ", " + lon),
                "Put these coordinates in the Google/Apple map's search bar to get precise location of you phone.");
//        getOpenStreetMapLink(lat, lon)
        ch.getSender().sendNow(msg);
    ...
    public void sendLastKnownLocation() {
        ...
        String msg = String.format("%s: Lat: %s Lon: %s\nTime: %s\n%s\n%s",
                ...
                Utils.getGeoURI(lat, lon), Utils.getOpenStreetMapLink(lat, lon));
        ch.getSender().sendNow(msg);
    }
